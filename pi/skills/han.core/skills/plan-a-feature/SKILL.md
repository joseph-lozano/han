...
- **All sub-agents in this skill run on openai-codex/gpt-5.5:low.** When launching any Agent tool call in this skill, pass `model: "openai-codex/gpt-5.5:low"`.
...
**When launching each agent, pass `model: "openai-codex/gpt-5.5:low"` to the Agent tool. Use domain-scoped briefs — do not hand every agent the full set of artifacts.** Pass each agent only the spec sections relevant to its domain plus pointers, and instruct it to read the rest on demand only if its domain needs it. Default mapping:
...
Launch the `project-manager` agent in **synthesis mode** (pass `model: "openai-codex/gpt-5.5:low"`). Provide it with:
