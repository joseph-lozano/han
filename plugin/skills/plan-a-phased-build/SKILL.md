...
description: >
  Splits source context into vertical-slice build phases where each phase is
  independently demonstrable to a real user and builds on the previous phase.
  Use when the user wants to plan, sequence, phase, slice, break down, order, or
  roadmap the build of a feature, capability, system, or initiative, including
  "split this into phases", "what should we build first", "create a build plan",
  or "turn this into vertical slices". Accepts gap analyses, PRDs, design docs,
  feature specs, conversation notes, ADRs, requirements, or inline descriptions,
  and produces build-phase-outline.md with stable phase IDs, traceability,
  preconditions, and demoable outcomes. Does not produce implementation detail,
  decide unspecified behavior, perform gap analysis, record ADRs, or refine an
  existing plan.
